...
wink.cache = (function()
{
	 ...
	 * @param {string} resources.type The type can be 'js' or 'css' or 'json'
	 * @param {string} resources.callback 
	...
	cache.load = function(resources, onload, options)
	{
		...
		try
		{
			console.log("opt.storage="+opts.storage);
	...
	var _load = function(resources, callback, fromCache)
	{
		...
		for (i = 0; i < l; i++)
		{
			...
			_getResource(urlm, type, version, expires, callback);
		}
	};
	 ...
	 * @param {function} callback The callback
	 */
	var _loadJson = function(url, content, callback)
	{
		callback();
	};
	...
	var _getResource = function (url, type, version, expires, callback)
	{
		/*if (navigator.onLine){
	 ...
	 * resource with an xhr and inserts the retrieved
	 ...
	 */
	var _getFreshResource = function(url, type, version, expires, callback)
	{
			console.log("getFreshResource");
	 ...
	 * @param {function} expires Expires value
	 */
	var _getLocalResource = function(url, type, version, expires, callback)
	{
		console.log("getLocalResource");
